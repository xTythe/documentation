# Credit Enhancement Vault (CEV)

Credit Enhancement Vaults are the only module on Tythe that enables undercollateralized lending. CEVs are ERC-4626 vaults where LPs provide collateral top-offs to back verified, high-TCT borrowers; allowing borrowers to access loans at lower collateral requirements than the market normally demands.
...
* **Total Fixed Deposit (TFD):** The vault's total capital capacity in dollar ($) terms. The vault remains inactive and closed to borrowers until TFD is reached in full.
* **Capacity Policy:** Permanent and cannot be changed post-deployment. Two options:\
...
* **Fixed Deployment Fee (Optional):** A one-time fee paid by LPs at entry. Set by the curator at deployment. Compensates the curator for vault setup and ongoing management.
...
* **Risk Premium Share:** Proportional cut of Risk Premiums collected from borrower top-off events, distributed according to LP share.
