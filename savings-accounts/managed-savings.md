...
The risk boundaries are set by the protocol, not by the manager. Within them, the manager optimises for yield; it cannot move the account outside the risk-bounded limits that define it.

#### Why it stays conservative

Reaching for more yield does not mean taking open-ended risk. Three things keep Managed Savings conservative:

The sleeve is delta-neutral, so its volatile positions are hedged and hold their dollar value regardless of market direction. The Synclear options strategy runs here at a conservative exposure, deliberately held well below the unbounded level used in the Options Yield Fund. And the whole allocation sits within protocol-defined risk limits, with a protocol-owned reserve sized to absorb the modeled tail of the sleeve.

The account is still designed to hold its value. It carries more risk than Open Savings, but that risk is bounded and reserve-covered rather than open-ended.

#### How to use Managed Savings

**Deposit.** Once verified, deposit any supported stablecoin, including USDC, USDT, USDe, and USDS. In return you receive the Managed Savings token, an ERC-4626 vault share that represents your position.
